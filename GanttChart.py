...
def makeColourGradient(frequency1, frequency2, frequency3,
	...
	grad = [wx.Colour(*[int(math.sin(f*i + p) * width + center) for f, p in fp]) for i in range(len)]
	return grad
...
if __name__ == '__main__':
	def GetData():
		...
		interp = []
		for i in range(20):
			data.append( [t + i*10 for t in range(0, 60*60, 7*60)] )
			if i % 5 == 1:
				data[-1].insert( (i//3) + 1, data[-1][i//3] + 0.05 )
			interp.append( [((t + i*10)%100)//10 for t in range(0, 60*60, 7*60)] )
		return data, interp

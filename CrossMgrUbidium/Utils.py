...
def getHomeDir():
	...
	try:
		if os.path.basename(homedir) == '.CrossMgrUbidium':
			homedir = os.path.join( os.path.dirname(homedir), '.CrossMgrUbidiumApp' )
	except Exception:
		pass
...
if 'WXMAC' in wx.Platform:
	...
	if os.path.isdir( os.path.join(topdirName, 'CrossMgrUbidiumImages') ):
		dirName = topdirName
	else:
	...
	if 'CrossMgrUbidium?' in os.path.basename(dirName):
		dirName = os.path.dirname(dirName)
	if not os.path.isdir( os.path.join(dirName, 'CrossMgrUbidiumImages') ):
		dirName = os.path.dirname(dirName)

	if os.path.isdir( os.path.join(dirName, 'CrossMgrUbidiumImages') ):
		pass
	elif os.path.isdir( '/usr/local/CrossMgrUbidiumImages' ):
		dirName = '/usr/local'

imageFolder = os.path.join(dirName, 'CrossMgrUbidiumImages')
htmlFolder = os.path.join(dirName, 'CrossMgrUbidiumHtml')
helpFolder = os.path.join(dirName, 'CrossMgrHtmlUbidiumDoc')

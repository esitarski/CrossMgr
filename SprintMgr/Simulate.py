	...
	while tse:
		...
		for i, r in enumerate(results):
			if r and r != competition.state.OpenRider:
				print ( '%2d: %s' % (i+1, r) )
		print ( '\n'.join( 'DNF: %s' % r for r in dnfs ) )
		print ( '\n'.join( 'DQ: %s' % r for r in dqs ) )
		...
		for i, p in enumerate(start.startPositions):
			print ( 'Start Position %d: %s' % (i+1, competition.state.labels[p]) )
		raw_input()
		...
		if random.randint(1,5) == 1:
			...
			if random.randint(1, 2) == 1:
				...
				print ( 'With relegation of %s=%s' % (competition.state.labels[id], id) )
			...
			for i, c in enumerate(places):
				print ( 'Finish Position %d:  %s' % (i+1, competition.state.labels[c]) )
		print ()
